...
The content files are converted/copied as html and stored in a _to_html directory.
...
All templates can be found recursively up the tree, except _dir.
A _dir template must be defined at the level. Absense of one
suggests that nothing should be shown for that directory request.
This way, we can have branches for /, and /blog/, but not for /blog/a/b/;
meaning that /blog/a/b/ should not have a branch page.
...
We use filepath.Walk which walks files in sorted order.
To support multiple templates, all templates must be organized in lexical order.
This way, templates which are depended upon by others are loaded first.

Old paths (e.g. /project/*) are handled using a redirect.
To support this, we keep a rewrite map in a json file, and update this for any new path.

At startup time, we walk the directory, read in *.page.* files and snapshot the server.
...
In terms of the code structure, a OS process runs an engine. That engine holds a number of
server objects, one for each base uri that the engine serves. When a request comes into the
engine, we match the hostname to a server and dispatch to the appropriate server.
Each server is self-contained and can be removed or added to the engine as needed.
Also, each server can reload itself - though this reload doesn't accomodate config changes.
The engine however can be reloaded to read the config changes.

An engine can serve requests for multiple host names. If the host name matches, it is
dispatched to the appropriate server.

At startup, we will write pid to a file. This will allow easy shutdown of server.
A shell script will auto-restart the server if it dies.

Note that a tag is:
  - all lower case
  - no spaces
  - words separated by dashes
A function called "_title" is added to the template funcmap to convert to title case.
...
To use, create a form with the destination ${dynamicPathPrefix}message (e.g. /d/message), 
and put all the parameters you need in there.
...
serveFile will look for NAME.gz, and if it finds it, decipher the 
content type from the regular file, and serve the gzip version,
...
If patterns are seen in the file, replace them with replacements 
stored locally. This way, some replacements can be done at build time,
as opposed to only doing within javascript.
...
/*
---- LIVE ugorji.net, intrigal.com, ... ----

To make it easy to add share buttons or adsense to any page,
we use divs which are replaced at runtime with the actual contents.
  <div class="share-facebook"></div>
  <div class="share-googleplus"></div>
  <div class="share-twitter"></div>

  <div class="adsense-300x250"></div>
  <div class="adsense-728x90"></div>
  <div class="adsense-160x600"></div>

Ads should only be placed on the leaf pages.
These ones have content.

Feeds can be generated for any pages that match some tags and prefixes.
However, only show links to the "primary tags". This way, folks can
subscribe to "social", "technology" postings, etc.

Site will be tracked via access log and google analytics.
